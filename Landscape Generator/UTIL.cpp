...
Couleur::Couleur(int rr, int rg, int rb):m_r{rr},m_g{rg},m_b{rb}
{}

void Couleur::afficher()
{
   std::cout<<"r:"<<m_r<<" g:"<<m_g<<" b:"<<m_b<<std::endl;
}
...
{
    double c;
    return c=std::rand()%(maxi-mini)+mini;
}

int aleatoire_int(int mini, int maxi)
{
    int c;
    return c=rand()%(maxi-mini)+mini;
}
